...
class FolderService {
    static async getOrCreateFolder(userId, name, parentId) {
    ...
    static async getFolderSizes(folderIds) {
        const sizes = await prisma.file.groupBy({
            by: ['folderId'],
            where: { folderId: { in: folderIds }, isDeleted: false },
            _sum: { fileSize: true },
        });
        return Object.fromEntries(sizes.map(s => [s.folderId, s._sum.fileSize?.toString() ?? '0']));
    }
    static async renameFolder(folderId, userId, newName) {
    ...
    static async softDeleteFolderRecursive(folderId, userId, deletedAt) {
    ...
    static async restoreFolderRecursive(folderId, userId, deletedAt) {
        ...
        const fileWhere = { folderId, userId, isDeleted: true };
        if (deletedAt) {
        ...
        const childWhere = { parentId: folderId, userId, isDeleted: true };
        if (deletedAt) {
    ...
    static async permanentDeleteFolderRecursive(folderId, userId) {
    ...
    static async getTrashFolders(userId, page = 1, limit = 20) {
    ...
    static async getTrashFolderContents(userId, folderId) {
        const folder = await prisma.folder.findFirst({
            where: { id: folderId, userId, isDeleted: true },
        });
        if (!folder)
    ...
    static async getFolderBreadcrumb(folderId) {
        const breadcrumb = [];
    ...
    static async moveFolder(folderId, userId, targetParentId) {
    ...
    static async bulkDelete(folderIds, userId) {
        const now = new Date();
        let deleted = 0;
