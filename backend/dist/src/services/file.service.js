...
class FileService {
    ...
    static async getUserFiles(userId, page = 1, limit = 20, includeDeleted = false, folderId, sortBy = 'uploadDate', sortDir = 'desc') {
        const where = { userId, folderId: folderId ?? null };
        if (!includeDeleted) {
    ...
    static async getTrashFiles(userId, page = 1, limit = 20) {
    ...
    static async permanentDeleteFile(fileId, userId) {
        ...
        await notification_service_1.NotificationService.createForAdmins(`File pending deletion: "${file.originalName}"`, `A user permanently deleted a file. It will be purged in 30 days unless recovered by an admin.`);
        return { success: true };
    }
    ...
    static async hasSharedAccess(fileId, userId, requiredPermission = 'VIEW') {
        ...
        if (file?.folderId) {
            ...
            while (currentFolderId !== null) {
                ...
                if (folderShare) {
                    if (requiredPermission === 'DOWNLOAD')
                        return folderShare.permission === 'DOWNLOAD';
                    return true;
                }
                const folderRec = await prisma.folder.findUnique({ where: { id: currentFolderId }, select: { parentId: true } });
                currentFolderId = folderRec?.parentId ?? null;
            }
        }
        return false;
    }
    static async renameFile(fileId, userId, newName, userRole = 'USER') {
        const file = await prisma.file.findFirst({
            where: { id: fileId, userId, isDeleted: false },
        });
        if (!file)
            throw new Error('File not found');
        const oldExt = path_1.default.extname(file.originalName).toLowerCase();
        const newExt = path_1.default.extname(newName).toLowerCase();
    ...
    static async moveFile(fileId, userId, targetFolderId) {
        const file = await prisma.file.findFirst({
            where: { id: fileId, userId, isDeleted: false },
        });
        if (!file)
    ...
    static async searchFiles(userId, query, fileType, dateFrom, dateTo, sizeMinMB, sizeMaxMB, tagIds) {
        const fileWhere = {
            userId,
            isDeleted: false,
        ...
        if (dateFrom || dateTo) {
            ...
            if (dateFrom)
                fileWhere.uploadDate.gte = new Date(dateFrom);
            if (dateTo)
                fileWhere.uploadDate.lte = new Date(dateTo);
        }
        if (sizeMinMB !== undefined || sizeMaxMB !== undefined) {
            fileWhere.fileSize = {};
            if (sizeMinMB !== undefined)
                fileWhere.fileSize.gte = BigInt(Math.round(sizeMinMB * 1024 * 1024));
            if (sizeMaxMB !== undefined)
                fileWhere.fileSize.lte = BigInt(Math.round(sizeMaxMB * 1024 * 1024));
        }
    ...
    static async bulkMove(fileIds, userId, targetFolderId) {
        if (targetFolderId !== null) {
            const folder = await prisma.folder.findFirst({ where: { id: targetFolderId, userId } });
            if (!folder)
                throw new Error('Target folder not found');
    ...
    static async getBulkFiles(fileIds, userId) {
    ...
    static async checkDuplicate(userId, fileHash) {
        const existing = await prisma.file.findFirst({
    ...
    static async deleteAllInFolder(userId, folderId) {
    ...
    static async getRecentFiles(userId, limit = 20) {
        const files = await prisma.file.findMany({
    ...
    static async getStarredFiles(userId, page = 1, limit = 20) {
    ...
    static async toggleStar(fileId, userId) {
        const file = await prisma.file.findFirst({
            where: { id: fileId, userId },
        });
        if (!file)
    ...
    static async getDuplicateFiles(userId, page = 1, limit = 50) {
        ...
        const hashes = duplicateGroups.map(g => g.fileHash);
        if (hashes.length === 0) {
    ...
    static async getFileAccessLog(fileId, ownerUserId, page = 1, limit = 20) {
