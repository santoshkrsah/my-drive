...
export class UserService {
  ...
  static async updateUser(id: number, input: UpdateUserInput) {
    ...
    if (input.storageQuota !== undefined) data.storageQuota = input.storageQuota;
    if (input.password) data.passwordHash = await bcrypt.hash(input.password, 12);
    if (input.maxUploadSize !== undefined) data.maxUploadSize = input.maxUploadSize;
    if (input.maxFilesPerUpload !== undefined) data.maxFilesPerUpload = input.maxFilesPerUpload;
    if (input.allowedExtensions !== undefined) data.allowedExtensions = input.allowedExtensions;
  ...
  static async deleteUser(id: number, permanent: boolean = false, delayMinutes: number = 60) {
    if (permanent) {
    ...
    await prisma.user.update({
      where: { id },
      data: { status: UserStatus.DELETED, scheduledDeletionAt: new Date(Date.now() + delayMinutes * 60 * 1000) },
    });
  ...
  static async unbanUser(id: number) {
    return prisma.user.update({
      where: { id },
  ...
  static async getAllUsers(page: number = 1, limit: number = 20, search?: string) {
    ...
    const storageMap = new Map<number, bigint>();
